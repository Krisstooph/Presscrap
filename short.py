...
urls_to_open = {
    ...
    'MECZYKI': 'https://www.meczyki.pl/newsy/',
}
...
def prepare_article_div(article_title, article_header, article_url, article_color):
    return f'<div class="column" style="border-radius: 25px; border: 2px solid #999999; padding: 10px; margin: 0 5px 25px 0; float: left; width: 100%; background-color: #{article_color};"><a href="{article_url}" style="text-decoration: none;"><h2 style="text-align: center; font-size: 30px; font-family: Helvetica, sans-serif; color: black;">{article_title}</h2><p style="font-size: 20px; font-family: Arial, Helvetica, sans-serif; font-weight: bold; color: #555555;">{article_header}</p></a></div>'


def create_output_file():
    html_header = '<html><header><title>Prasówka : short</title></header><body>'
    html_end = '</body></html>'
    ...
    file.write(football_news_block)
    file.write(aviation_news_block)
    file.write(html_end)
    file.close()


def short_header(article_header):
    if len(article_header) > 300:
        return f'{article_header[:300]}...'
...
def prepare_articles_pch24():
    ...
    for article_div in articles_divs:
        ...
        article_header = short_header(article_header)
...
def write_articles_for_wykop(soup, articles_number, color):
    ...
    for article_div in articles_divs:
        ...
        article_header = article_div.find('div', attrs={'class': 'content'}).find('section',
...
def get_time_diff(current_duration, target_duration):
    start = datetime.strptime(current_duration, '%H:%M')
    end = datetime.strptime(target_duration, '%H:%M')
    delta = end - start

    if delta.days == -1:
        return (start - end).__str__()[:-3]

    return delta.__str__()[:-3]
...
def prepare_day_info_block():
    ...
    # Based on https://kalendarz.livecity.pl/sunrise-sunset/%C5%81%C3%B3d%C5%BA
    longest_day_duration = '16:37'
    shortest_day_duration = '07:43'

    text_compare_to_longest_day = get_time_diff(text_day_duration, longest_day_duration)
    text_compare_to_shortest_day = get_time_diff(text_day_duration, shortest_day_duration)

    return f'<p style="font-size: 10px; margin-bottom: -20px;">{PRESSCRAP_SHORT_VERSION}</p><div style="font-family: Tahoma;"><h1>Dziś jest {text_date}</h1><h2>{text_time}</h2><p>Wschód słońca: <b>{text_sun_rise}</b></p><p>Zachód słońca: <b>{text_sun_set}</b></p><p>Dzień trwa: <b>{text_day_duration}</b> godzin</p><p>Krótszy od najdłuższego o: <b>{text_compare_to_longest_day}</b> godzin</p><p>Dłuższy od najkrótszego o: <b>{text_compare_to_shortest_day}</b> godzin</p></div>'
...
def prepare_next_matches_block():
    ...
    html += f'<h3><a href="https://www.flashscore.pl/pilka-nozna/hiszpania/laliga/tabela/"><b>La Liga</b></a>&nbsp;&nbsp;&nbsp;<a href="https://www.flashscore.pl/pilka-nozna/polska/pko-bp-ekstraklasa/tabela/"><b>Ekstraklasa</b></a></h3>'
...
def prepare_semi_left_news_block():
    html = '<div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</div><hr><hr><br><br><ul style="font-family: Tahoma; font-size: 22px;">'
    articles_number = 3
    ...
    html += '<h7 style="font-size: 11px;"><b>tvp info - niedostępne, zmiany na stronie</b></h7></br>'
    ...
    article_list = list(dict(sorted(articles_and_comments.items(), key=lambda item: item[1], reverse=True)).keys())[
                   :articles_number]

    html += '<h7 style="font-size: 11px;"><b>wPolityce</b></h7>'

    for article in article_list:
        title = article.find('span', attrs={'class': 'long-title'}).text
        url = f'{urls_to_open["WPOLITYCE"]}{article.find("a")["href"]}'

        html += f'<li><a href="{url}">{title}</a></li>'

    soup = get_soup_from_page(urls_to_open['NIEZALEZNA'])
    article_list = soup.find_all('div', attrs={'class': 'col'})[1].find_all('div', attrs={'class': 'item-news'})[
                   :articles_number]
    html += '<h7 style="font-size: 11px;"><b>niezależna</b></h7>'

    for article in article_list:
        title = article.find('div', attrs={'class': 'news-title-title'}).text.strip()
...
def prepare_football_news():
    articles_number = 5
    html = '</br><h5><b>Piłka nożna</b></h5><ul style="font-family: Tahoma; font-size: 18px;">'

    soup = get_soup_from_page(urls_to_open['MECZYKI'])
    article_list = soup.find_all('a', attrs={'class': 'news-slide-href'})[:articles_number]

    for article in article_list:
        title = article.find('h3', attrs={'class': 'news-title'}).text.strip()
        url = f'https://www.meczyki.pl{article["href"]}'
...
def prepare_wind_forecast():
    html_0 = '<script src="https://www.windfinder.com/widget/forecast/js/czarnocin_lodz_voivodeship_poland?unit_wave=m&unit_rain=mm&unit_temperature=c&unit_wind=kts&unit_pressure=hPa&days=4&show_day=1&show_pressure=0&show_waves=0&show_clouds=0"></script><noscript><a rel="nofollow" href="https://www.windfinder.com/forecast/czarnocin_lodz_voivodeship_poland?utm_source=forecast&utm_medium=web&utm_campaign=homepageweather&utm_content=noscript-forecast">Wind forecast for Czarnocin</a> provided by <a rel="nofollow" href="https://www.windfinder.com?utm_source=forecast&utm_medium=web&utm_campaign=homepageweather&utm_content=noscript-logo">windfinder.com</a></noscript>'
    html_1 = '<script src="https://www.windfinder.com/widget/forecast/js/zalew_sulejowski?unit_wave=m&unit_rain=mm&unit_temperature=c&unit_wind=kts&unit_pressure=hPa&days=4&show_day=1&show_pressure=0&show_waves=0&show_clouds=0"></script><noscript><a rel="nofollow" href="https://www.windfinder.com/forecast/zalew_sulejowski?utm_source=forecast&utm_medium=web&utm_campaign=homepageweather&utm_content=noscript-forecast">Wind forecast for Zalew Sulejowski</a> provided by <a rel="nofollow" href="https://www.windfinder.com?utm_source=forecast&utm_medium=web&utm_campaign=homepageweather&utm_content=noscript-logo">windfinder.com</a></noscript>'
    html_2 = '<script src="https://www.windfinder.com/widget/forecast/js/jeziorsko?unit_wave=m&unit_rain=mm&unit_temperature=c&unit_wind=kts&unit_pressure=hPa&days=4&show_day=1&show_pressure=0&show_waves=0&show_clouds=0"></script><noscript><a rel="nofollow" href="https://www.windfinder.com/forecast/jeziorsko?utm_source=forecast&utm_medium=web&utm_campaign=homepageweather&utm_content=noscript-forecast">Wind forecast for Jeziorsko</a> provided by <a rel="nofollow" href="https://www.windfinder.com?utm_source=forecast&utm_medium=web&utm_campaign=homepageweather&utm_content=noscript-logo">windfinder.com</a></noscript>'
    html_3 = '<script src="https://www.windfinder.com/widget/forecast/js/jastarnia?unit_wave=m&unit_rain=mm&unit_temperature=c&unit_wind=kts&unit_pressure=hPa&days=4&show_day=1&show_pressure=0&show_clouds=0"></script><noscript><a rel="nofollow" href="https://www.windfinder.com/forecast/jastarnia?utm_source=forecast&utm_medium=web&utm_campaign=homepageweather&utm_content=noscript-forecast">Wind forecast for Jastarnia</a> provided by <a rel="nofollow" href="https://www.windfinder.com?utm_source=forecast&utm_medium=web&utm_campaign=homepageweather&utm_content=noscript-logo">windfinder.com</a></noscript>'

    return f'{html_0}{html_1}{html_2}{html_3}'
...
def get_aviation_crash_info():
    ...
    try:
        ...
        print('[Aviation Herald]: Failed to respond...')
        return html
...
def prepare_aviation_news():
    ...
    sections = soup.find('div', attrs={'class': 'content_def'}).find_all_next('section')[:3]
